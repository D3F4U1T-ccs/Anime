...
function Home() {
  ...
  const fetchPage = async (p: number, replace = false) => {
    ...
    try {
      ...
      const url = `/api/anime?limit=${PAGE_SIZE}&offset=${offset}`;
  ...
  const ALL_TYPES = [
    "Боевые искусства", "История", "Меха", "Полиция", "Ужасы", "Вампиры", "Демоны", "Космос",
    "Музыка", "Самураи", "Спорт", "Школа", "Военное", "Игры", "Магия", "Пародия", "Сёнен", "Суперсила",
  ];
                ...
                return (
                  <div key={anime._id} onClick={() => navigate(`/anime/${anime.slug}`)} className="cursor-pointer flex flex-col items-center group">
                    <div className="relative z-10 w-[215px] h-[215px] rounded-full overflow-hidden shadow-lg">
                      <img src={anime.thumbnail} alt={anime.nameEn} className="w-full h-full object-cover scale-110 group-hover:scale-115 transition-transform duration-300" />
                    </div>
